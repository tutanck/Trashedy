...
public class GetProfileService{
	...
	@WebService(
			webServlet = @WebServlet(urlPatterns={"/user/profile"}),
			optionalIn={"uther"},
	...
	public static JSONObject getProfile(
		...
		if(params.has("uther")) 
			THINGS.getOne(JSONRefiner.renameKeys(
					JSONRefiner.slice(params,"uther"),"uther->_id"), 
					UserProfileCore.collection);
		else{
			user = THINGS.getOne(JSONRefiner.renameKeys(
					JSONRefiner.slice(UserSessionDB.clarifyParams(params), "uid"),"uid->_id"),
					UserProfileCore.collection);
